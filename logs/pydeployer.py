...
This version:
 - Works locally (example, build, test, deploy, rollback)
 - Automatically clones or updates a Git repository before running
 - Logs everything into /logs
 - No AWS or Docker yet
"""
...
def clone_or_update_repo(repo_url: str):
    ...
    if not repo_dir.exists():
        ...
        info(f"🔄 Updating existing repository '{repo_name}' ...")
...
# === Main entrypoint ===
def run_stage(stage_name: str, repo_url: str = None):
    config = load_config()
    clean_old_logs()
...
if __name__ == "__main__":
    ...
    run_stage(args.stage, args.repo)
